...
3) 自动确认并“最终输出”：
   ...
   - 不做“二选一确认题”：即使用户要求“只回复 1A 2A”这类选择题，只要已成功读取 diff，也必须直接输出最终可粘贴的提交信息文本。
   - 不询问“这些要不要算进同一条提交”：默认合并为同一条提交；subject 只写一个核心结果，其余相关改动写入 body 分行说明。
...
- `scope` 优先使用中文模块名（如：登录/版本/界面/依赖）。
 - 若无法从 diff 100% 确定单一模块，直接使用 `全部范围`，不得为 scope 发起确认提问。
...
- 不要把无关改动合并为一个提交说明。
 - 当改动看起来可能应该拆分多个提交时：仍输出一条提交信息（不提问确认）；在 body 末尾增加一行建议（例如：`建议：若需拆分提交，可按“X / Y”分开提交`）。
